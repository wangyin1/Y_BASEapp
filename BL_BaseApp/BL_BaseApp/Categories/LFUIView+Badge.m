...
- (void)badgeInit
{
    if (nil == self.lf_badge) {
        ...
        self.lf_badge.backgroundColor = [UIColor redColor];
        self.lf_badge.textColor = [UIColor whiteColor];
        self.lf_badge.text = @"";
        self.lf_badge.layer.cornerRadius = self.bounds.size.width/ 2.F;
...
- (void)configBadge:(LFBadgeType)type style:(LFBadgeStyle)style sizeType:(LFBadgeSizeType)sizeType value:(NSInteger)value{
    ...
    switch (type) {
        ...
        {
            self.lf_badge.text = @"";
            self.lf_badge.bounds = CGRectMake(0, 0, LF_BADGE_VIEW_REDDOT_WIDTH, LF_BADGE_VIEW_REDDOT_WIDTH);
        }
            break;
