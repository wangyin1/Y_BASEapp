...
-(CGSize)sizeWithFont:(UIFont*)font width:(CGFloat)width{
    ...
    return [self boundingRectWithSize:CGSizeMake(width, MAXFLOAT) options:NSStringDrawingUsesLineFragmentOrigin|NSStringDrawingUsesFontLeading|NSStringDrawingTruncatesLastVisibleLine attributes:@{NSFontAttributeName:font} context:nil].size;
}

-(CGSize)sizeWithFont:(UIFont*)font width:(CGFloat)width lineSpacing:(NSInteger)lineSpace{
    NSMutableParagraphStyle *paragraphStyle = [[NSMutableParagraphStyle alloc]init];
    [paragraphStyle setLineSpacing:lineSpace];
    
    return [self boundingRectWithSize:CGSizeMake(width, MAXFLOAT) options:NSStringDrawingUsesLineFragmentOrigin|NSStringDrawingUsesFontLeading|NSStringDrawingTruncatesLastVisibleLine attributes:@{NSFontAttributeName:font,NSParagraphStyleAttributeName:paragraphStyle} context:nil].size;
}
...
-(NSAttributedString*)attributedStringWithLineSpacing:(NSInteger)lineSpace font:(UIFont *)font{
...
-(NSAttributedString*)attributedStringWithcharacterSpacing:(NSInteger)characterSpacing{
    NSMutableAttributedString *attributedString = [[NSMutableAttributedString alloc]initWithString:self];
    [attributedString addAttribute:NSKernAttributeName value:@(characterSpacing) range:NSMakeRange(0, self.length)];
    return attributedString;
}
