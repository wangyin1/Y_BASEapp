...
+ (instancetype )shareData{
    ...
    if (!hud) {
        ...
        hud.styleColor = MBProgressHUDColorBlack;
    }
    return hud;
}
+ (void)_setDefaltColorStyle:(MBProgressHUDColor)color{
    [MBProgressHUD shareData].styleColor = color;
}

+(void)showString:(NSString *)str duration:(CGFloat)duration
{
    if (!str||str.length==0) {
        return;
    }
...
- (void)commonInit {
    ...
    self.styleColor = [MBProgressHUD shareData].styleColor;
...
- (void)setStyleColor:(MBProgressHUDColor)styleColor{
    _styleColor = styleColor;
    if (styleColor==0) {
        self.bezelView.style = 1;
        self.contentColor = [UIColor colorWithWhite:0.f alpha:0.7f];
    }else{
        self.bezelView.style=0;
        self.bezelView.color = [[UIColor blackColor] colorWithAlphaComponent:.7];
