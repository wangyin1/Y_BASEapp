...
- (void)viewDidLoad {
    ...
    UIBarButtonItem *left = [[UIBarButtonItem alloc] initWithTitle:@"取消" style:1 target:self action:@selector(cancelBtnAction:)];
    left.tintColor = [UIColor blackColor];
    self.navigationItem.leftBarButtonItem = left;
    
    UIBarButtonItem *right = [[UIBarButtonItem alloc] initWithTitle:@"确定" style:1 target:self action:@selector(okBtnAction:)];
...
- (void)setupNavBar {
    ...
    NSArray *navBarCons2 = [NSLayoutConstraint constraintsWithVisualFormat:[NSString stringWithFormat:@"V:|[navBar(==64)]"] options:0 metrics:nil views:NSDictionaryOfVariableBindings(navBar)];
...
- (void)setupPhotoListView {
    ...
    //    NSArray *cons1 = [NSLayoutConstraint constraintsWithVisualFormat:[NSString stringWithFormat:@"V:|-%d-[collectionView]|",kDevice_Is_iPhoneX?88:64] options:0 metrics:nil views:NSDictionaryOfVariableBindings(collectionView)];
...
- (void)setupGroupView {
    ...
    if (@available(iOS 11.0, *)) {
        [photoGroupView setContentInsetAdjustmentBehavior:2];
    } else {
        // Fallback on earlier versions
    }
    ...
    //    [self.view addConstraints:cons1];
    //    [self.view addConstraints:cons2];
    //    self.navigationItem.titleView = photoGroupView;
}
