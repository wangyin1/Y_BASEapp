...
@property (nonatomic, copy) DismissBlock deletBlock;
...
+ (instancetype)showFromImageView:(UIImageView *)imageView withAutoImages:(NSArray *)images atIndex:(NSInteger)index deletBlock:(DismissBlock)delet;{
    HUPhotoBrowser *hup  = [HUPhotoBrowser showFromImageView:imageView withURLStrings:images atIndex:index];
    hup.deletBlock = delet;
    return hup;
}
...
- (void)setupToolBar {
    ...
    UIButton *delet = [UIButton buttonWithType:UIButtonTypeCustom];
    delet.frame = CGRectMake(8, 1, 50, 28);
    delet.layer.cornerRadius = 2;
    [delet setBackgroundColor:[[UIColor blackColor]colorWithAlphaComponent:0.4]];
    [delet setTitle:@"删除" forState:UIControlStateNormal];
    [delet setTitleColor:[UIColor whiteColor] forState:UIControlStateNormal];
    delet.titleLabel.font = [UIFont systemFontOfSize:13];
    [delet addTarget:self action:@selector(deletImae) forControlEvents:UIControlEventTouchUpInside];
    [_toolBar addSubview:delet];
...
- (void)deletImae{
    ...
    if (self.URLStrings&&self.URLStrings.count>0) {
        NSMutableArray *urls = self.URLStrings.mutableCopy;
        image = urls[_currentPage];
        [urls removeObjectAtIndex:_currentPage];
        self.URLStrings = urls;
    }
    if (self.images&&self.images.count>0) {
        NSMutableArray *urls = self.images.mutableCopy;
        image = urls[_currentPage];
        [urls removeObjectAtIndex:_currentPage];
        self.images = urls;
    }
    self.imagesCount = MAX(self.URLStrings.count, self.images.count);
    [self resetCountLabWithIndex:MIN(_currentPage, _imagesCount-1)+1];
    [self animateImageViewAtIndex:MIN(_currentPage, _imagesCount-1)];
    if (self.deletBlock) {
        self.deletBlock(image, index);
    }
    if (_imagesCount==0) {
        [self dismiss];
    }
